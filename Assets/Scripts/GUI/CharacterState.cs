...
public class CharacterState : MonoBehaviour
{
    ...
    private Color _color_active = new Color(0.5f, 1f, 0.5f);
    private Color _color_ready = new Color(1f, 1f, 1f);
    private Color _color_cooldownPhase = new Color(0.5f, 0.5f, 0.5f);
            ...
            foreach (GameObject skillSprite in skills)
            {
                if (skill.SkillName == skillSprite.name)
                {
                    ...
                    if (skill.State == Skill.SkillState.InExecution || skill.State == Skill.SkillState.Active)
                        skillSprite.GetComponent<UISprite>().color = _color_active;

                    else if (skill.State != Skill.SkillState.Ready)
                    {
                        skillSprite.GetComponent<UISprite>().color = _color_cooldownPhase;
                        buttons[skills.IndexOf(skillSprite)].GetComponent<UISprite>().color = _color_cooldownPhase;
                    }
                    else
                    {
                        skillSprite.GetComponent<UISprite>().color = _color_ready;
                        buttons[skills.IndexOf(skillSprite)].GetComponent<UISprite>().color = _color_ready;
                    }
                }
            }
        }
